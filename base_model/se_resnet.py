...
class Basic_Block(keras.Model):
    ...
    def __init__(self, filters, downsample=False, stride=1):
        ...
        self.se_resize = keras.layers.Reshape((1, 1, filters))
        self.se_fc1 = keras.layers.Dense(units=filters // 16, activation='relu',
                                         use_bias=False)
        self.se_fc2 = keras.layers.Dense(units=filters, activation='sigmoid',
                                         use_bias=False)
    ...
    def call(self, inputs, **kwargs):
        ...
        # se_block
        b = x
        x = self.se_globalpool(x)
        x = self.se_resize(x)
        x = self.se_fc1(x)
        x = self.se_fc2(x)
        x = keras.layers.Multiply()([b, x])
...
class Block(keras.Model):
    ...
    def __init__(self, filters, block_name,
        ...
        self.se_fc1 = keras.layers.Dense(units=4*filters//16, activation='relu',
                                         use_bias=False,name=conv_name+'se_fc1')
        self.se_fc2 = keras.layers.Dense(units=4 * filters, activation='sigmoid',
                                         use_bias=False, name=conv_name + 'se_fc2')
